...
function Calendar(isHijr,year,month,firstDay,lang,theme,tmout){
	...
	createAboutModal=function(){
		...
		cont.style.cssText='width:440px;height:300px;cursor:default; opacity:0.5;';
	...
	createCal=function(){
		...
		headerElm.style.cssText='height:40px; border-radius: 5px 5px 0px 0px; ';
		todayElm.style.cssText='margin:2px 20px 0px 0px; cursor:default; opacity:0.7;';
		yearValElm.style.cssText='cursor:default; opacity:0.57;';
		monthValElm.style.cssText='margin-bottom:8px;cursor:default; opacity:0.7;';
		...
		gridsElm.style.cssText=' opacity:0.7; border-radius:0px 0px 5px 5px; ';
		menuWrapElm.style.cssText='width:200px;';
	...
	updTodayLbl=function(){
		todayElm.innerHTML=isSmallScreen?dispDate.todayShortString():dispDate.todayString()
	},
	updHeader=function(){
	...
	createDates=function(){
		...
		for(let i=1;i<=ppdr+pcdr+pndr;i++){
			...
			grid.style.cssText='width:14.2857%;padding:2.2137% 0px';
	...
	applyTheme=function(){
		headerElm.className=headerElm.className.substring(0,headerElm.className.lastIndexOf('w3-'))+'w3-'
	},
...
HijriDate.prototype.todayShortString=function(){
	let t=this.getTime();this.setTime(Date.now());
	let	s=this.getWeekdayName()+', '+this.getDate()+' '+this.getMonthName()+' '+this.getFullYear()+' H';
	this.setTime(t);return Calendar.getDigit(s)
};
